...
# raw/2 class name (c0..c9) -> target folder mapping
RAW2_CLASS_TO_TARGET: Dict[str, str] = {
    "c0": "SafeDriving",
    "c1": "DangerousDriving",
    "c2": "DangerousDriving",
    "c3": "DangerousDriving",
...
def collect_raw2_grouped(raw_train_dir: Path) -> Dict[str, List[Path]]:
    """Aggregate imgs/train/cX into target buckets by RAW2_CLASS_TO_TARGET."""
    grouped: Dict[str, List[Path]] = {}
    if not raw_train_dir.exists():
        return grouped
    for d in sorted(p for p in raw_train_dir.iterdir() if p.is_dir()):
        target = RAW2_CLASS_TO_TARGET.get(d.name)
        if not target:
            continue
        imgs = list_images(d)
        if not imgs:
            continue
        grouped.setdefault(target, []).extend(imgs)
    return grouped
...
def run(workspace_root: Path, seed: int = 42, dry_run: bool = False, purge_target: bool = False, only_classes: List[str] | None = None, skip_existing: bool = False) -> None:
    raw2_root = workspace_root / "raw" / "2"
    raw_imgs_root = raw2_root / "imgs"
    ...
    if not grouped:
        raise RuntimeError("No images found in raw/2/imgs/train. Check paths and contents.")
    ...
    print("Split raw/2 into train/test/valid" + (" (dry-run)" if dry_run else ""))
    for cls in targets:
        s = summary[cls]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Split raw/2 imgs/train (c0..c9) into data/processed train/test/valid with mapping to 4 classes"
        )
    )
    ...
    parser.add_argument(
        "--purge-target",
        action="store_true",
        help="Clear target class dirs (DangerousDriving, Distracted, Object, SafeDriving) before copying",
    )
    parser.add_argument(
        "--only-classes",
        nargs="*",
        choices=["DangerousDriving", "Distracted", "Object", "SafeDriving"],
